...
class CalendarScreen extends StatefulWidget {
  ...
  List<String> getSuggestedTitles(List<Event> events) {
    ...
    final sortedTitles = frequencyMap.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
...
class _CalendarScreenState extends State<CalendarScreen> {
        ...
        return StatefulBuilder(builder: (ctx, setModalState) {
          Future<void> pickImage() async {
            ...
            if (pickedFile != null) {
              ...
              final savedImage =
              await File(pickedFile.path).copy('${dir.path}/${p.basename(pickedFile.path)}');
              setModalState(() {
  ...
  void _scheduleNotification(Event event) async {
    ...
    // Get local timezone using Dart's native support
    final String timeZoneName = DateTime.now().timeZoneName;
    try {
      tz.setLocalLocation(tz.getLocation(timeZoneName));
    } catch (e) {
      // fallback in case timezone is not recognized
  ...
  Widget build(BuildContext context) {
    ...
    provider.events.where((event) => isSameDay(event.dateTime, _selectedDay ?? _focusedDay)).toList();
                ...
                return ListTile(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      ...
                      if (event.people.isNotEmpty) Text("People: ${event.people.join(', ')}"),
                    ],
                  ),
                  trailing: IconButton(
